...
import { TrademarkProduct, TrademarkUpdate, TrademarkUpdateSchema } from '@/libs/schema/trademark'
...
const Update: React.FC<UpdateProps> = ({ unit, open, handleClose }) => {
  ...
  const { mutate } = api.trademark.update.useMutation()
  const utils = api.useContext()

  const { control, handleSubmit, reset } = useForm<TrademarkUpdate>({
    defaultValues: {
      name: unit?.name as string,
      country: unit?.country as string,
      image: unit?.image as string,
      introduce: unit?.introduce as string,
    },
    values: {
    ...
    resolver: zodResolver(TrademarkUpdateSchema),
  })
  ...
  const onSubmit: SubmitHandler<TrademarkUpdate> = async (data) => {
  ...
  return (
    ...
    >
      <BoxContainer>
        ...
        >
          <Typography fontWeight={700}>{t('update.title')}</Typography>
          ...
          <Stack direction="row" spacing={2}>
            ...
            >
              ...
              <AvatarWhenEit>
                <Camera>
                  <MuiImage
                    src={image ? image : (unit?.image as string)}
                    width={150}
                    height={150}
                    alt="choose avatar"
                    onLoad={() => {
